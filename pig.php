<?php /*
=================================================
=== PIG version 0.3.2.4 from 23 Dec 2015
=== Ecommerce Inc.
=== Updated by: James Holt
=== Changes from pig-0.3.2.3:
=================================================
=== Altered Control Panel determination to fix issue with web6xx servers
=== Removed inactive servers from cp 1-5 arrays
=== Thanks to Yuliya Gordeeva for bug submission
=== Thanks to Evgen Movchan (and Vlad) for CP/server association assistance
=================================================
...
$controlpanelarr = array(
	1 => array("iis5","iis12","iisn101","iisn102","iisn103","iisn104","iisn105","web57","web236","web250","web257"),
	...
	4 => array("iisn401","iisn402","iisn403","iisn404","iisn405","iisn406","iisn407","iisn408","iisn409","iisn410","iisn411","iisn412","iisn413","web245","web255","web260"),
	5 => array("iis132","iis133","iis134","iis138","iis139","iis140","web241","web242","web243","web247","web248","web249","web261")
);
...
$phpvars=array(
	...
	"5.5"=>array(
		"ini"=>"/hsphere/shared/php55/etc/php.ini",
		"cgi"=>array(
			"server"=>"/hsphere/shared/php55/bin/php-cgi",
			"local"=>'php55.cgi',
			"code"=>"#!/bin/sh\nexport PHP_FCGI_CHILDREN=3\nexec /hsphere/shared/php55/bin/php-cgi "
		),
		"htaccess"=>array(
			"code"=>"AddHandler php55 .php\nAction php55 /cgi-bin/php55.cgi\n\n",
			"search"=>"@(?<!#)AddHandler\s+php55\s+\.php(?:\s+\.html?)?\s+Action\s+php55\s+/cgi-bin/php55\.cgi\n*@ims"
		)
	)
...
$phpIniHeader = 
'; | Generated by PIG v'.VERSION.' for PHP '.$phpverselected.' |
';
...
//	LogMsg("PlaceCustomPhpIni: $domain: $rootdir.DS.$phpinipath", true, WARN );
	LogMsg("PlaceCustomPhpIni: $domain: $phpinipath", true, WARN );
	if( $set_postmaster )
		switch (PHP_OS)
		{
...
//	$doNothing ? nop() : file_put_contents( $rootdir.DS.$phpinipath, $php_ini_contents );
	$doNothing ? nop() : file_put_contents( $phpinipath, $php_ini_contents );
}
...
//				file_put_contents($path.'/'.$phpvars[$phpverselected]['cgi']['local'],$phpvars[$phpverselected]['cgi']['code'] . $rootdir . DS . $phpinipath);
				file_put_contents($path.'/'.$phpvars[$phpverselected]['cgi']['local'],$phpvars[$phpverselected]['cgi']['code'] . $phpinipath);
...
*/
	if($serverNumber>=900)
		$CP = floor($serverNumber/100);
	elseif($serverNumber>=600)
		$CP = floor($serverNumber/100)+2;
	elseif($serverNumber>=300)
		$CP = floor($serverNumber/100)+3;
	else
		for( $i = 1; $i <= 5; $i++ )
...
function GetServerPhpIniPath($phpvars)
{
	...
	if($phpverselected!=""){
	...
	}elseif(PHP_MAJOR_VERSION=='5' && PHP_MINOR_VERSION=='5' && PHP_OS==LINUX){
		$serverphpinipath=$phpvars['5.5']['ini'];
	    $serverphpinifolder = dirname($serverphpinipath);
		...
		$serverphpinipath=$phpvars['4.4']['ini'];
	    $serverphpinifolder = dirname($serverphpinipath);
...
if(isset($_POST['submit'])){
		...
		// downloading ioncube if not already installed or not containing appropriate .so file
		if( isset($_POST['ioncube']) && (!file_exists($rootdir."/ioncube") || !file_exists($rootdir."/ioncube/ioncube_loader_lin_".$phpverselected.".so")) ){
			$disabled_functions=explode(',', ini_get('disable_functions'));
			  $ionfname="ioncube.zip";
	          print ("Downloading $ionfname : ");
		      $ch = curl_init("http://downloads2.ioncube.com/loader_downloads/ioncube_loaders_lin_" . $_POST['ioncube'] . ".zip");
		      $fp = fopen($rootdir  . "/$ionfname" , "w");
		      curl_setopt($ch, CURLOPT_FILE, $fp);
		      curl_setopt($ch, CURLOPT_HEADER, 0);
		      curl_exec($ch);
		      curl_close($ch);
		      fclose($fp);
		      if ( filesize($rootdir  . "/$ionfname" ) > 1000 ) 
		      { 
	            print ("<font color='green'><div class='bold'>[ Ok ]</div></font><br>");
				print ("Extracting $ionfname : ");
				if(class_exists('ZipArchive')){
				  $zip = new ZipArchive;
				  $res = $zip->open("$rootdir/$ionfname");
				  if ($res === TRUE) {
				    $zip->extractTo("$rootdir/");
				    $zip->close();
				    print ("<font color='green'><div class='bold'>[ Ok ]</div></font><br>");
				  } else {
				    print ("<font color='red'><div class='bold'>[ Failed ]</div></font><br>");
				  }
				}elseif(!in_array('exec',$disabled_functions)){
		          print `cd $rootdir  && unzip -o $rootdir/$ionfname  > /dev/null && chmod 755 -vR $rootdir/ioncube/*  > /dev/null`;
		          print ("<font color='green'><div class='bold'>[ Ok ]</div></font><br>");
		        }elseif(!in_array('system',$disabled_functions)){
		          $result=system('cd $rootdir  && unzip -o $rootdir/$ionfname  > /dev/null && chmod 755 -vR $rootdir/ioncube/*  > /dev/null');
		          if($result===false)
		            print ("<font color='green'><div class='bold'>[ Ok ]</div></font><br>");
		          else
		            print ("<font color='red'><div class='bold'>[ Failed ]</div></font><br>");
				...
				unlink("$rootdir/$ionfname");
		      }else{
	            print ("<font color='red'><div class='bold'>[ Failed ]</div></font><br>");
		      }
		}//end downloading ioncube
		else{
		  print ("Downloading $ionfname : <font><div class='bold'>[ Skipped ]</div></font><br>");
		}
...
//			    UpdateParameter("zend_extension",$rootdir . "/ioncube/ioncube_loader_lin_".PHP_MAJOR_VERSION.".".PHP_MINOR_VERSION.".so");
		...
		if($controlpanel > 8 && in_array($phpverselected, array("5.3","5.4","5.5")))
		     ...
		     if ( PHP_OS==LINUX ) 
		       { 
		        if  (strstr($phpinfo, 'ioncube'))
		            print (" [x] IonCube: Ioncube loader already enabled<br>");
